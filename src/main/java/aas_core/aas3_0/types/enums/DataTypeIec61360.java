...
public enum DataTypeIec61360 {
   ...
   *
   * <p>In IEC61360-1 (2017) only URI is supported. An IRI type allows in particular to express an
   * URL or an URI.
   */
  IRI,
  /**
   ...
   *
   * <p>ICID values are value conformant to an IRDI, where the delimiter between RAI and ID is “#”
   * while the delimiter between DI and VI is confined to “##”
   *
   * <p>ISO29002_IRDI values are values containing a global identifier that identifies an
   * administrated item in a registry. The structure of this identifier complies with identifier
   * syntax defined in ISO/TS 29002-5. The identifier shall fulfil the requirements specified in
   * ISO/TS 29002-5 for an "international registration data identifier" (IRDI).
   */
  IRDI,
   ...
   *
   * <p>Example from IEC 61360-1:2017: "13:20:00-05:00" is the [TIME] representation of: 1.20 p.m.
   * for Eastern Standard Time, which is 5 hours behind Coordinated Universal Time (UTC).
   */
  TIME,
  /**
   * values containing a time, conformant to ISO 8601:2004 but restricted to what is allowed in the
   ...
   *
   * <p>In IEC61360 binary is for a sequence of bits, each bit being represented by “0” and “1”
   * only. A binary is a blob but a blob may also contain other source code.
   */
  BLOB
}
