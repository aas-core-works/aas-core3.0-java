...
public class Capability implements ICapability {
  ...
  /** Iterate over {@link Capability#extensions}, if set, and otherwise return an empty iterator. */
  public Iterable<IExtension> overExtensionsOrEmpty() {
    return getExtensions().orElseGet(Collections::emptyList);
  }

  /**
   * Iterate over {@link Capability#displayName}, if set, and otherwise return an empty iterator.
   */
  ...
  /**
   * Iterate over {@link Capability#description}, if set, and otherwise return an empty iterator.
   */
  ...
  /**
   * Iterate over {@link Capability#supplementalSemanticIds}, if set, and otherwise return an empty
   * iterator.
   */
  ...
  /** Iterate over {@link Capability#qualifiers}, if set, and otherwise return an empty iterator. */
  public Iterable<IQualifier> overQualifiersOrEmpty() {
    return getQualifiers().orElseGet(Collections::emptyList);
  }

  /**
   * Iterate over {@link Capability#embeddedDataSpecifications}, if set, and otherwise return an
   * empty iterator.
   */
  ...
  private class CapabilityIterable implements Iterable<IClass> {
    ...
    private Stream<IClass> stream() {
      ...
      if (extensions != null) {
        memberStream = Stream.concat(memberStream, Capability.this.extensions.stream());
      }

      if (displayName != null) {
        memberStream = Stream.concat(memberStream, Capability.this.displayName.stream());
      }

      if (description != null) {
        memberStream = Stream.concat(memberStream, Capability.this.description.stream());
      }

      if (semanticId != null) {
        memberStream = Stream.concat(memberStream, Stream.<IClass>of(Capability.this.semanticId));
      }

      if (supplementalSemanticIds != null) {
        memberStream =
            Stream.concat(memberStream, Capability.this.supplementalSemanticIds.stream());
      }

      if (qualifiers != null) {
        memberStream = Stream.concat(memberStream, Capability.this.qualifiers.stream());
      }

      if (embeddedDataSpecifications != null) {
        memberStream =
            Stream.concat(memberStream, Capability.this.embeddedDataSpecifications.stream());
      }
            ...
            Stream.concat(
                ...
                Stream.concat(
                    ...
                    StreamSupport.stream(
                        Capability.this.semanticId.descend().spliterator(), false)));
      }
