...
public class Property implements IProperty {
  ...
  /** Iterate over {@link Property#extensions}, if set, and otherwise return an empty iterator. */
  public Iterable<IExtension> overExtensionsOrEmpty() {
    return getExtensions().orElseGet(Collections::emptyList);
  }

  /** Iterate over {@link Property#displayName}, if set, and otherwise return an empty iterator. */
  public Iterable<ILangStringNameType> overDisplayNameOrEmpty() {
    return getDisplayName().orElseGet(Collections::emptyList);
  }

  /** Iterate over {@link Property#description}, if set, and otherwise return an empty iterator. */
  public Iterable<ILangStringTextType> overDescriptionOrEmpty() {
    return getDescription().orElseGet(Collections::emptyList);
  }

  /**
   * Iterate over {@link Property#supplementalSemanticIds}, if set, and otherwise return an empty
   * iterator.
   */
  ...
  /** Iterate over {@link Property#qualifiers}, if set, and otherwise return an empty iterator. */
  public Iterable<IQualifier> overQualifiersOrEmpty() {
    return getQualifiers().orElseGet(Collections::emptyList);
  }

  /**
   * Iterate over {@link Property#embeddedDataSpecifications}, if set, and otherwise return an empty
   * iterator.
   */
  ...
  private class PropertyIterable implements Iterable<IClass> {
    ...
    private Stream<IClass> stream() {
      ...
      if (extensions != null) {
        memberStream = Stream.concat(memberStream, Property.this.extensions.stream());
      }

      if (displayName != null) {
        memberStream = Stream.concat(memberStream, Property.this.displayName.stream());
      }

      if (description != null) {
        memberStream = Stream.concat(memberStream, Property.this.description.stream());
      }

      if (semanticId != null) {
        memberStream = Stream.concat(memberStream, Stream.<IClass>of(Property.this.semanticId));
      }

      if (supplementalSemanticIds != null) {
        memberStream = Stream.concat(memberStream, Property.this.supplementalSemanticIds.stream());
      }

      if (qualifiers != null) {
        memberStream = Stream.concat(memberStream, Property.this.qualifiers.stream());
      }

      if (embeddedDataSpecifications != null) {
        memberStream =
            Stream.concat(memberStream, Property.this.embeddedDataSpecifications.stream());
      }

      if (valueId != null) {
        memberStream = Stream.concat(memberStream, Stream.<IClass>of(Property.this.valueId));
      }
