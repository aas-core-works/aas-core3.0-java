...
public class Range implements IRange {
  ...
  /** Iterate over {@link Range#extensions}, if set, and otherwise return an empty iterator. */
  public Iterable<IExtension> overExtensionsOrEmpty() {
    return getExtensions().orElseGet(Collections::emptyList);
  }

  /** Iterate over {@link Range#displayName}, if set, and otherwise return an empty iterator. */
  public Iterable<ILangStringNameType> overDisplayNameOrEmpty() {
    return getDisplayName().orElseGet(Collections::emptyList);
  }

  /** Iterate over {@link Range#description}, if set, and otherwise return an empty iterator. */
  public Iterable<ILangStringTextType> overDescriptionOrEmpty() {
    return getDescription().orElseGet(Collections::emptyList);
  }

  /**
   * Iterate over {@link Range#supplementalSemanticIds}, if set, and otherwise return an empty
   * iterator.
   */
  ...
  /** Iterate over {@link Range#qualifiers}, if set, and otherwise return an empty iterator. */
  public Iterable<IQualifier> overQualifiersOrEmpty() {
    return getQualifiers().orElseGet(Collections::emptyList);
  }

  /**
   * Iterate over {@link Range#embeddedDataSpecifications}, if set, and otherwise return an empty
   * iterator.
   */
  ...
  private class RangeIterable implements Iterable<IClass> {
    ...
    private Stream<IClass> stream() {
      ...
      if (extensions != null) {
        memberStream = Stream.concat(memberStream, Range.this.extensions.stream());
      }

      if (displayName != null) {
        memberStream = Stream.concat(memberStream, Range.this.displayName.stream());
      }

      if (description != null) {
        memberStream = Stream.concat(memberStream, Range.this.description.stream());
      }

      if (semanticId != null) {
        memberStream = Stream.concat(memberStream, Stream.<IClass>of(Range.this.semanticId));
      }

      if (supplementalSemanticIds != null) {
        memberStream = Stream.concat(memberStream, Range.this.supplementalSemanticIds.stream());
      }

      if (qualifiers != null) {
        memberStream = Stream.concat(memberStream, Range.this.qualifiers.stream());
      }

      if (embeddedDataSpecifications != null) {
        memberStream = Stream.concat(memberStream, Range.this.embeddedDataSpecifications.stream());
      }
